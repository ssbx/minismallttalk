...
static SDL_Color _bg = { 0x29, 0x2A, 0x24, SDL_ALPHA_OPAQUE };
static SDL_Color white = { 0xF8, 0xF8, 0xE2, SDL_ALPHA_OPAQUE };
//static SDL_Color black = { 0x00, 0x00, 0x00, SDL_ALPHA_OPAQUE };
static SDL_Color brown = { 0x49, 0x48, 0x3E, SDL_ALPHA_OPAQUE };

#include "editbox.h"

#define DEFAULT_PTSIZE  14.5f
...
static void HandleKeyDown(Scene *scene, SDL_Event *event)
{
  ...
  switch (event->key.key) {
    ...
    case SDLK_H:
      SDL_FRect editRect = scene->edit->rect;
      if (toggle_editbox_size) {
        toggle_editbox_size = false;
        editRect.w *= 2.0f;
        editRect.h *= 2.0f;
      } else {
        toggle_editbox_size = true;
        editRect.w /= 2.0f;
        editRect.h /= 2.0f;
      }
      scene->textRect = editRect;
      EditBox_SetRect(scene->edit, &editRect);
      break;
    case SDLK_U:
...
int main(int argc, char *argv[])
{
  ...
  /* Create a window */
  scene.window = SDL_CreateWindow("showfont demo", WIDTH, HEIGHT,
    ...
    while (SDL_PollEvent(&event)) {
      ...
      switch (event.type) {
        ...
        case SDL_EVENT_WINDOW_RESIZED:
          int w, h;
          float border = 10.0f;
          float bordertop = 25.0f;
          if (!SDL_GetWindowSize(scene.window, &w, &h)) abort();
          SDL_FRect editRect;
          editRect.w = (float) w - 2.0f * border;
          editRect.h = (float) h - border - bordertop;
          editRect.x = 0.0f + border;
          editRect.y = 0.0f + bordertop;
          scene.textRect = editRect;
          EditBox_SetRect(scene.edit, &editRect);
          break;
        default:
